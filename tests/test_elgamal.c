...
static void test_homomorphic_add_post_mergeinbox_attack(const secp256k1_context *ctx);
...
static void test_homomorphic_add_point_at_infinity(const secp256k1_context *ctx)
{
    ...
    EXPECT(secp256k1_elgamal_encrypt(ctx, &c1_a, &c2_a, &pubkey, amount_a, r) == 1);

    /* Encrypt amount_b with randomness -r: Enc(amount_b; -r) */
    EXPECT(secp256k1_elgamal_encrypt(ctx, &c1_b, &c2_b, &pubkey, amount_b, neg_r) == 1);
     ...
     */
    ret = secp256k1_elgamal_add(ctx, &result_c1, &result_c2,
                                &c1_a, &c2_a, &c1_b, &c2_b);
...
static void test_homomorphic_add_post_mergeinbox_attack(const secp256k1_context *ctx)
{
    ...
    printf("Running test: post-MergeInbox inbox locking attack (TOB-RIPCTXR-5 variant)...\n");
    ...
    printf("  Expected: 0 (failure — malicious Send rejected at library level)\n");
     ...
     * sufficient when combined with correct return value checking (TOB-RIPCTXR-14).
     *
