...
/** NOTE: These signatures must match exactly what is in
...
extern int derive_ipa_round_challenge(const secp256k1_context *ctx,
                                      unsigned char u_out[32],
                                      const unsigned char last_challenge[32],
                                      const secp256k1_pubkey *L,
                                      const secp256k1_pubkey *R);

extern int fold_generators(const secp256k1_context *ctx,
                           secp256k1_pubkey *final_point,
                           const secp256k1_pubkey *generators,
                           const unsigned char *u_flat,
                           const unsigned char *uinv_flat, size_t n,
                           size_t rounds, int is_H);

extern int apply_ipa_folding_to_P(
    const secp256k1_context *ctx, secp256k1_pubkey *P,
    const secp256k1_pubkey *L_vec, const secp256k1_pubkey *R_vec,
    const unsigned char *u_flat, const unsigned char *uinv_flat, size_t rounds);

extern int secp256k1_bulletproof_ipa_dot(const secp256k1_context *ctx,
                                         unsigned char *out,
...
static int test_ipa_verify_explicit(
    ...
    const secp256k1_pubkey *H_vec, const secp256k1_pubkey *U,
    const secp256k1_pubkey *P_in, const secp256k1_pubkey *L_vec,
    const secp256k1_pubkey *R_vec, size_t n, const unsigned char a_final[32],
    const unsigned char b_final[32], const unsigned char ux[32],
...
int main(void)
{
  ...
  EXPECT(secp256k1_mpt_get_generator_vector(ctx, G, N_BITS,
                                            (unsigned char *)"G", 1) == 1);
  EXPECT(secp256k1_mpt_get_generator_vector(ctx, H, N_BITS,
                                            (unsigned char *)"H", 1) == 1);
  EXPECT(secp256k1_mpt_get_generator_vector(ctx, &U, 1, (unsigned char *)"BP_U",
                                            4) == 1);
  ...
  int res = secp256k1_bulletproof_run_ipa_prover(
      ctx, &U, G, H, a_vec, b_vec, /* These get folded in-place */
      N_BITS, ipa_transcript_id, ux, L, R, IPA_ROUNDS, &rounds_out, a_final,
      b_final);
  EXPECT(res == 1);
  ...
  int ok = test_ipa_verify_explicit(ctx, G0, H0, &U, &P, L, R, N_BITS, a_final,
                                    b_final, ux, ipa_transcript_id);
