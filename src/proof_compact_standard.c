...
static void compute_compact_std_challenge(
    ...
    const secp256k1_pubkey *T_PCm,
    const secp256k1_pubkey *K1,
    const secp256k1_pubkey *T_PCb,
    const secp256k1_pubkey *K2,
...
#define SER(pk_ptr) do { \
    ...
    /* Statement */
    for (size_t i = 0; i < n; i++) SER(&Pk_vec[i]);
    SER(C1);
    for (size_t i = 0; i < n; i++) SER(&C2_vec[i]);
    SER(PC_m);
    SER(pk_A);
    SER(PC_b);
    SER(C1_rem);
    ...
    /* Commitments */
    SER(T1);
    for (size_t i = 0; i < n; i++) SER(&T2_vec[i]);
    SER(T_PCm);
    SER(K1);
    SER(T_PCb);
    SER(K2);
...
int secp256k1_compact_standard_prove(
    const secp256k1_context *ctx, unsigned char *proof_out,
    uint64_t amount, uint64_t remainder,
    ...
    const unsigned char *context_id)
{
    ...
    if (!secp256k1_ec_seckey_verify(ctx, sk_A)) return 0;
    if (!secp256k1_ec_seckey_verify(ctx, r_b)) return 0;
    ...
    if (!generate_random_scalar(ctx, beta))    goto cleanup;
    if (!generate_random_scalar(ctx, gamma))   goto cleanup;
    if (!generate_random_scalar(ctx, delta))   goto cleanup;
    if (!generate_random_scalar(ctx, epsilon)) goto cleanup;

    /* 2. Compute commitments */

    /* T1 = alpha*G */
    if (!secp256k1_ec_pubkey_create(ctx, &T1, alpha)) goto cleanup;

    /* T_{2,i} = alpha*pk_i + beta*G */
    {
        secp256k1_pubkey betaG;
        if (!secp256k1_ec_pubkey_create(ctx, &betaG, beta)) goto cleanup;
        for (size_t i = 0; i < n; i++) {
            secp256k1_pubkey aPk = Pk_vec[i];
            if (!secp256k1_ec_pubkey_tweak_mul(ctx, &aPk, alpha)) goto cleanup;
            const secp256k1_pubkey *pts[2] = {&aPk, &betaG};
            if (!secp256k1_ec_pubkey_combine(ctx, &T2_vec[i], pts, 2)) goto cleanup;
        }
    }
    ...
    /* T_PCm = alpha*G + beta*H */
    {
        ...
        if (!secp256k1_ec_pubkey_create(ctx, &alphaG, alpha)) goto cleanup;
        betaH = H;
        if (!secp256k1_ec_pubkey_tweak_mul(ctx, &betaH, beta)) goto cleanup;
        const secp256k1_pubkey *pts[2] = {&alphaG, &betaH};
        if (!secp256k1_ec_pubkey_combine(ctx, &T_PCm, pts, 2)) goto cleanup;
    }

    /* K1 = gamma*G */
    if (!secp256k1_ec_pubkey_create(ctx, &K1, gamma)) goto cleanup;

    /* T_PCb = delta*G + epsilon*H */
    {
        secp256k1_pubkey deltaG, epsH;
        if (!secp256k1_ec_pubkey_create(ctx, &deltaG, delta)) goto cleanup;
        epsH = H;
        if (!secp256k1_ec_pubkey_tweak_mul(ctx, &epsH, epsilon)) goto cleanup;
        const secp256k1_pubkey *pts[2] = {&deltaG, &epsH};
        if (!secp256k1_ec_pubkey_combine(ctx, &T_PCb, pts, 2)) goto cleanup;
    }
    ...
    /* K2 = gamma*C1_rem + epsilon*G */
    {
        ...
        gC1r = *C1_rem;
        if (!secp256k1_ec_pubkey_tweak_mul(ctx, &gC1r, gamma)) goto cleanup;
        if (!secp256k1_ec_pubkey_create(ctx, &epsG, epsilon)) goto cleanup;
        const secp256k1_pubkey *pts[2] = {&gC1r, &epsG};
        if (!secp256k1_ec_pubkey_combine(ctx, &K2, pts, 2)) goto cleanup;
    }

    /* 3. Challenge */
    compute_compact_std_challenge(ctx, e, n, Pk_vec, C1, C2_vec, PC_m,
                                  pk_A, PC_b, C1_rem, C2_rem,
                                  &T1, T2_vec, &T_PCm, &K1, &T_PCb, &K2,
                                  context_id);
    ...
    if (!secp256k1_ec_seckey_tweak_add(ctx, z_r, term)) goto cleanup;

    /* z_m = beta + e*m */
    memcpy(z_m, beta, 32);
    memcpy(term, m_scalar, 32);
    if (!secp256k1_ec_seckey_tweak_mul(ctx, term, e)) goto cleanup;
    if (!secp256k1_ec_seckey_tweak_add(ctx, z_m, term)) goto cleanup;

    /* z_sk = gamma + e*sk_A */
    memcpy(z_sk, gamma, 32);
    memcpy(term, sk_A, 32);
    if (!secp256k1_ec_seckey_tweak_mul(ctx, term, e)) goto cleanup;
    if (!secp256k1_ec_seckey_tweak_add(ctx, z_sk, term)) goto cleanup;

    /* z_rb = delta + e*r_b */
    memcpy(z_rb, delta, 32);
    memcpy(term, r_b, 32);
    if (!secp256k1_ec_seckey_tweak_mul(ctx, term, e)) goto cleanup;
    if (!secp256k1_ec_seckey_tweak_add(ctx, z_rb, term)) goto cleanup;

    /* z_v = epsilon + e*v */
    memcpy(z_v, epsilon, 32);
    memcpy(term, v_scalar, 32);
    if (!secp256k1_ec_seckey_tweak_mul(ctx, term, e)) goto cleanup;
    if (!secp256k1_ec_seckey_tweak_add(ctx, z_v, term)) goto cleanup;
...
int secp256k1_compact_standard_verify(
    const secp256k1_context *ctx, const unsigned char *proof,
    size_t n,
    const secp256k1_pubkey *C1, const secp256k1_pubkey *C2_vec,
    const secp256k1_pubkey *Pk_vec,
    const secp256k1_pubkey *PC_m,
    ...
    const unsigned char *context_id)
{
    ...
    if (!secp256k1_ec_seckey_verify(ctx, z_r))  return 0;
    if (!secp256k1_ec_seckey_verify(ctx, z_m))  return 0;
    if (!secp256k1_ec_seckey_verify(ctx, z_sk)) return 0;
    if (!secp256k1_ec_seckey_verify(ctx, z_rb)) return 0;
    if (!secp256k1_ec_seckey_verify(ctx, z_v))  return 0;
    ...
    /* T1 = z_r*G - e*C1 */
    {
        ...
        if (!secp256k1_ec_pubkey_create(ctx, &zrG, z_r)) goto cleanup;
        eC1 = *C1;
        if (!secp256k1_ec_pubkey_tweak_mul(ctx, &eC1, neg_e)) goto cleanup;
    ...
    /* T_{2,i} = z_r*pk_i + z_m*G - e*C_{2,i} */
    {
        ...
        if (!secp256k1_ec_pubkey_create(ctx, &zmG, z_m)) goto cleanup;
        for (size_t i = 0; i < n; i++) {
            secp256k1_pubkey zrPk, eC2;
            zrPk = Pk_vec[i];
            if (!secp256k1_ec_pubkey_tweak_mul(ctx, &zrPk, z_r)) goto cleanup;
            eC2 = C2_vec[i];
            if (!secp256k1_ec_pubkey_tweak_mul(ctx, &eC2, neg_e)) goto cleanup;
            const secp256k1_pubkey *pts[3] = {&zrPk, &zmG, &eC2};
            if (!secp256k1_ec_pubkey_combine(ctx, &T2_vec[i], pts, 3)) goto cleanup;
        }
    }
    ...
    /* T_PCm = z_r*G + z_m*H - e*PC_m */
    {
        ...
        if (!secp256k1_ec_pubkey_create(ctx, &zrG, z_r)) goto cleanup;
        zmH = H;
        if (!secp256k1_ec_pubkey_tweak_mul(ctx, &zmH, z_m)) goto cleanup;
        ePCm = *PC_m;
        if (!secp256k1_ec_pubkey_tweak_mul(ctx, &ePCm, neg_e)) goto cleanup;
        const secp256k1_pubkey *pts[3] = {&zrG, &zmH, &ePCm};
        if (!secp256k1_ec_pubkey_combine(ctx, &T_PCm, pts, 3)) goto cleanup;
    }

    /* K1 = z_sk*G - e*pk_A */
    {
        secp256k1_pubkey zskG, ePk;
        if (!secp256k1_ec_pubkey_create(ctx, &zskG, z_sk)) goto cleanup;
        ePk = *pk_A;
        if (!secp256k1_ec_pubkey_tweak_mul(ctx, &ePk, neg_e)) goto cleanup;
        const secp256k1_pubkey *pts[2] = {&zskG, &ePk};
        if (!secp256k1_ec_pubkey_combine(ctx, &K1, pts, 2)) goto cleanup;
    }

    /* T_PCb = z_rb*G + z_v*H - e*PC_b */
    {
        secp256k1_pubkey zrbG, zvH, ePCb;
        if (!secp256k1_ec_pubkey_create(ctx, &zrbG, z_rb)) goto cleanup;
        zvH = H;
        if (!secp256k1_ec_pubkey_tweak_mul(ctx, &zvH, z_v)) goto cleanup;
        ePCb = *PC_b;
        if (!secp256k1_ec_pubkey_tweak_mul(ctx, &ePCb, neg_e)) goto cleanup;
        const secp256k1_pubkey *pts[3] = {&zrbG, &zvH, &ePCb};
        if (!secp256k1_ec_pubkey_combine(ctx, &T_PCb, pts, 3)) goto cleanup;
    }
    ...
    /* K2 = z_sk*C1_rem + z_v*G - e*C2_rem */
    {
        ...
        zskC1r = *C1_rem;
        if (!secp256k1_ec_pubkey_tweak_mul(ctx, &zskC1r, z_sk)) goto cleanup;
        if (!secp256k1_ec_pubkey_create(ctx, &zvG, z_v)) goto cleanup;
        eC2r = *C2_rem;
        if (!secp256k1_ec_pubkey_tweak_mul(ctx, &eC2r, neg_e)) goto cleanup;
        const secp256k1_pubkey *pts[3] = {&zskC1r, &zvG, &eC2r};
        if (!secp256k1_ec_pubkey_combine(ctx, &K2, pts, 3)) goto cleanup;
    }

    /* 3. Recompute challenge */
    compute_compact_std_challenge(ctx, e_prime, n, Pk_vec, C1, C2_vec, PC_m,
                                  pk_A, PC_b, C1_rem, C2_rem,
                                  &T1, T2_vec, &T_PCm, &K1, &T_PCb, &K2,
                                  context_id);
