/**
 * @file proof_link.c
 * @brief Zero-Knowledge Proof Linking ElGamal Ciphertexts and Pedersen
 * Commitments.
 *
 * This module implements a Sigma protocol to prove that an ElGamal ciphertext
 * and a Pedersen commitment encode the same underlying plaintext value \f$ m
 * \f$, without revealing \f$ m \f$ or the blinding factors.
 *
 * @details
 * **Statement:**
 ...
 *
 * 2. **Challenge:**
 * \f[ e = H(\text{"MPT_ELGAMAL_PEDERSEN_LINK"} \parallel C_1 \parallel C_2
 * \parallel P \parallel PC_m \parallel T_1 \parallel T_2 \parallel T_3
 * \parallel \dots) \f]
 *
 * 3. **Response:**
 ...
 *
 * **Security Context:**
 * This proof prevents "bait-and-switch" attacks where a user sends a valid
 * range proof for a small amount (e.g., 10) but updates the ledger balance with
 * a large or negative amount (e.g., 1,000,000 or -5). It binds the two
...
int secp256k1_elgamal_pedersen_link_prove(
    const secp256k1_context *ctx, unsigned char *proof,
    const secp256k1_pubkey *c1, const secp256k1_pubkey *c2,
    const secp256k1_pubkey *pk, const secp256k1_pubkey *pcm, uint64_t amount,
    ...
    const unsigned char *context_id)
{
  ...
  /* 1. Generate Nonces */
  if (!generate_random_scalar(ctx, km))
    goto cleanup;
  if (!generate_random_scalar(ctx, kr))
    goto cleanup;
  if (!generate_random_scalar(ctx, krho))
    goto cleanup;
  ...
  if (!secp256k1_ec_pubkey_tweak_mul(ctx, &krPk, kr))
    goto cleanup; // kr*Pk
  ...
  if (!secp256k1_ec_pubkey_tweak_mul(ctx, &krhoH, krho))
    goto cleanup; // krho*H
  ...
  if (!secp256k1_ec_seckey_tweak_mul(ctx, term, e))
    goto cleanup;
  if (!secp256k1_ec_seckey_tweak_add(ctx, sm, term))
    goto cleanup;
  ...
  if (!secp256k1_ec_seckey_tweak_mul(ctx, term, e))
    goto cleanup;
  if (!secp256k1_ec_seckey_tweak_add(ctx, sr, term))
    goto cleanup;
  ...
  if (!secp256k1_ec_seckey_tweak_mul(ctx, term, e))
    goto cleanup;
  if (!secp256k1_ec_seckey_tweak_add(ctx, srho, term))
    goto cleanup;
                                           ...
                                           const unsigned char *context_id)
{
  ...
  if (!secp256k1_ec_pubkey_parse(ctx, &T2, ptr, 33))
    goto cleanup;
  ptr += 33;
  if (!secp256k1_ec_pubkey_parse(ctx, &T3, ptr, 33))
    goto cleanup;
  ptr += 33;
  ...
  if (!secp256k1_ec_seckey_verify(ctx, sm))
    goto cleanup;
  if (!secp256k1_ec_seckey_verify(ctx, sr))
    goto cleanup;
  if (!secp256k1_ec_seckey_verify(ctx, srho))
    goto cleanup;
  ...
  /* Eq 2: sm * G + sr * Pk == T2 + e * C2 */
  {
    ...
    term = *pk;
    if (!secp256k1_ec_pubkey_tweak_mul(ctx, &term, sr))
      goto cleanup; // sr*Pk
  ...
  /* Eq 3: sm * G + srho * H == T3 + e * Pcm */
  {
    ...
    term = H;
    if (!secp256k1_ec_pubkey_tweak_mul(ctx, &term, srho))
      goto cleanup; // srho*H
