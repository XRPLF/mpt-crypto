 ...
 *
 * This module implements a generalized multi-statement Sigma protocol to prove
 * that
 ...
 * @details
 * **Statement:**
 * Given \f$ N \f$ ciphertexts \f$ (R_i, S_i) \f$ encrypted under public keys
 * \f$ P_i \f$, the prover demonstrates knowledge of scalars \f$ m \f$ and \f$
 * \{r_1, \dots, r_N\} \f$ such that for all \f$ i \in [1, N] \f$:
 * 1. \f$ R_i = r_i \cdot G \f$
 * 2. \f$ S_i = m \cdot G + r_i \cdot P_i \f$
 *
 * **Protocol (Shared Amount Nonce):**
 * The efficiency gain comes from reusing the random nonce for the amount (\f$
 * k_m \f$) across all \f$ N \f$ proofs, tying them mathematically to the same
 * value \f$ m \f$.
 *
 * 1. **Commitments:**
 ...
 *
 * 2. **Challenge:**
 * \f[ e = H(\dots \parallel T_m \parallel \{T_{r,G}^{(i)},
 * T_{r,P}^{(i)}\}_{i=1}^N \parallel \dots) \f]
 *
 * 3. **Responses:**
 ...
 * - \f$ s_m \cdot G + s_{r,i} \cdot P_i \stackrel{?}{=} T_m + T_{r,P}^{(i)} + e
 * \cdot S_i \f$
 *
 * **Security Context:**
 * This is crucial for "fan-out" transactions or auditing scenarios where a
 * single value must be proven correct against multiple encrypted destinations
 * simultaneously, ensuring consistency without revealing the value.
 *
...
compute_challenge_multi(const secp256k1_context *ctx, unsigned char *e_out,
                        ...
                        const secp256k1_pubkey *S, const secp256k1_pubkey *Pk,
...
int secp256k1_mpt_prove_same_plaintext_multi(
    ...
    uint64_t amount_m, size_t n, const secp256k1_pubkey *R,
    const secp256k1_pubkey *S, const secp256k1_pubkey *Pk,
    const unsigned char *r_array, const unsigned char *tx_id)
{
  ...
  *proof_len = required_len;
  ...
  if (!generate_random_scalar(ctx, k_m))
    goto cleanup;
  if (!secp256k1_ec_pubkey_create(ctx, &Tm, k_m))
    goto cleanup;
  ...
  for (i = 0; i < n; i++)
  {
    ...
    // kri -> TrG = kri * G
    if (!generate_random_scalar(ctx, kri))
      goto cleanup;
    if (!secp256k1_ec_pubkey_create(ctx, &TrG[i], kri))
      goto cleanup;

    // TrP = kri * Pk_i
    TrP[i] = Pk[i];
    if (!secp256k1_ec_pubkey_tweak_mul(ctx, &TrP[i], kri))
      goto cleanup;
  }
  ...
  // s_m = k_m + e * m
  {
    ...
    if (!secp256k1_ec_seckey_tweak_mul(ctx, m_scalar, e))
      goto cleanup; // m*e
    if (!secp256k1_ec_seckey_tweak_add(ctx, s_m, m_scalar))
      goto cleanup; // km + m*e
  }

  // Serialize Points first (Protocol Format)
  size_t len = 33;
  secp256k1_ec_pubkey_serialize(ctx, ptr, &len, &Tm, SECP256K1_EC_COMPRESSED);
  ptr += 33;
  for (i = 0; i < n; ++i)
  {
    secp256k1_ec_pubkey_serialize(ctx, ptr, &len, &TrG[i],
                                  SECP256K1_EC_COMPRESSED);
    ptr += 33;
  }
  for (i = 0; i < n; ++i)
  {
    secp256k1_ec_pubkey_serialize(ctx, ptr, &len, &TrP[i],
                                  SECP256K1_EC_COMPRESSED);
    ptr += 33;
  }
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (!secp256k1_ec_seckey_tweak_mul(ctx, term, e))
      goto cleanup; // r*e
    if (!secp256k1_ec_seckey_tweak_add(ctx, s_ri, term))
      goto cleanup; // k + r*e
...
int secp256k1_mpt_verify_same_plaintext_multi(
    const secp256k1_context *ctx, const unsigned char *proof, size_t proof_len,
    size_t n, const secp256k1_pubkey *R, const secp256k1_pubkey *S,
    const secp256k1_pubkey *Pk, const unsigned char *tx_id)
{
  ...
  /* 1. Deserialize */
  if (!secp256k1_ec_pubkey_parse(ctx, &Tm, ptr, 33))
    goto cleanup;
  ptr += 33;

  for (i = 0; i < n; ++i)
  {
    if (!secp256k1_ec_pubkey_parse(ctx, &TrG[i], ptr, 33))
      goto cleanup;
    ptr += 33;
  }
  for (i = 0; i < n; ++i)
  {
    if (!secp256k1_ec_pubkey_parse(ctx, &TrP[i], ptr, 33))
      goto cleanup;
    ptr += 33;
  }
  ...
  for (i = 0; i < n; ++i)
  {
    ...
    memcpy(s_ri, ptr, 32);
    ptr += 32;
    if (!secp256k1_ec_seckey_verify(ctx, s_ri))
      goto cleanup;
    ...
    if (!secp256k1_ec_pubkey_create(ctx, &LHS, s_ri))
      goto cleanup;
    ...
    /* LHS = SmG + s_ri * Pk_i */
    term = Pk[i];
    if (!secp256k1_ec_pubkey_tweak_mul(ctx, &term, s_ri))
      goto cleanup;
    pts[0] = &SmG;
    pts[1] = &term;
