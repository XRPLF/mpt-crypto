 ...
 * @brief Zero-Knowledge Proof of Plaintext Equality (1-to-N, Shared
 * Randomness).
 *
 * This module implements an optimized multi-recipient Sigma protocol to prove
 * that
 * \f$ N \f$ distinct ElGamal ciphertexts encrypt the **same** plaintext amount
 * \f$ m \f$ using the **same** randomness \f$ r \f$, but under different public
 * keys.
 *
 * @details
 * **Statement:**
 * Given a shared ephemeral key \f$ C_1 = r \cdot G \f$ and \f$ N \f$ components
 * \f$ C_{2,i} = m \cdot G + r \cdot P_i \f$ (where \f$ P_i \f$ is the public
 * key for recipient \f$ i \f$), the prover demonstrates knowledge of scalars
 * \f$ m, r \f$ such that all equations hold.
 *
 * **Optimization:**
 * Unlike the general "Multi-Statement" proof (where \f$ r_i \f$ varies), this
 * variant enforces \f$ r_1 = r_2 = \dots = r_N = r \f$. This reduces the proof
 * size significantly because we only need one response scalar \f$ s_r \f$ for
 * the randomness, rather than \f$ N \f$.
 *
 * **Protocol:**
 * 1. **Commitments:**
 * - \f$ T_r = k_r \cdot G \f$ (Commitment to shared randomness nonce)
 * - \f$ T_{m,i} = k_m \cdot G + k_r \cdot P_i \f$ (Commitment for each
 * recipient)
 *
 * 2. **Challenge:**
 * \f$ e = H(\dots \parallel C_1 \parallel \{C_{2,i}, P_i\} \parallel T_r
 * \parallel \{T_{m,i}\} \dots) \f$
 *
 * 3. **Responses:**
 ...
 * - For each \f$ i \f$: \f$ s_m \cdot G + s_r \cdot P_i \stackrel{?}{=} T_{m,i}
 * + e \cdot C_{2,i} \f$
 *
 * **Security Context:**
 * This is used for broadcast-style transactions where the sender wants to prove
 * to multiple auditors or recipients that they are all receiving the exact same
 * message/amount, efficiently.
 *
    ...
    const secp256k1_pubkey *Pk_vec, const unsigned char *context_id)
{
  ...
  if (!generate_random_scalar(ctx, k_m))
    goto cleanup;
  if (!generate_random_scalar(ctx, k_r))
    goto cleanup;

  /* 3. Compute Commitments */

  /* Tr = kr * G */
  if (!secp256k1_ec_pubkey_create(ctx, &Tr, k_r))
    goto cleanup;

  /* Tm_i = km * G + kr * Pk_i */
  secp256k1_pubkey kmG;
  if (!secp256k1_ec_pubkey_create(ctx, &kmG, k_m))
    goto cleanup;

  for (i = 0; i < n; i++)
  {
    secp256k1_pubkey krPk = Pk_vec[i];
    if (!secp256k1_ec_pubkey_tweak_mul(ctx, &krPk, k_r))
      goto cleanup; // kr * Pk

    const secp256k1_pubkey *pts[2] = {&kmG, &krPk};
    if (!secp256k1_ec_pubkey_combine(ctx, &Tm_vec[i], pts, 2))
      goto cleanup;
  }
  ...
  if (!secp256k1_ec_seckey_tweak_mul(ctx, term, e))
    goto cleanup;
  if (!secp256k1_ec_seckey_tweak_add(ctx, s_r, term))
    goto cleanup;
    ...
    const secp256k1_pubkey *Pk_vec, const unsigned char *context_id)
{
  ...
  // Parse Tr (33 bytes)
  if (!secp256k1_ec_pubkey_parse(ctx, &Tr, ptr, 33))
    goto cleanup;
  ptr += 33;
  ...
  for (i = 0; i < n; i++)
  {
    if (!secp256k1_ec_pubkey_parse(ctx, &Tm_vec[i], ptr, 33))
      goto cleanup;
    ptr += 33;
  }
  ...
  /* Eq 1: sr * G == Tr + e * C1 */
  {
    ...
    if (!secp256k1_ec_pubkey_create(ctx, &LHS, s_r))
      goto cleanup; // sr*G

    if (!secp256k1_ec_pubkey_tweak_mul(ctx, &eC1, e))
      goto cleanup; // e*C1
    const secp256k1_pubkey *pts[2] = {&Tr, &eC1};
    if (!secp256k1_ec_pubkey_combine(ctx, &RHS, pts, 2))
      goto cleanup; // Tr + e*C1
  ...
  /* Eq 2: For each i, sm * G + sr * Pk_i == Tm_i + e * C2_i */
  {
    ...
    for (i = 0; i < n; i++)
    {
      ...
      if (!secp256k1_ec_pubkey_tweak_mul(ctx, &srPk, s_r))
        goto cleanup;
      ...
      if (!secp256k1_ec_pubkey_tweak_mul(ctx, &eC2, e))
        goto cleanup;
