 ...
 * different ElGamal ciphertexts encrypt the **same** underlying plaintext
 * amount, potentially under different public keys and using different
 * randomness.
 *
 * @details
 * **Statement:**
 * Given two ciphertexts \f$ (R_1, S_1) \f$ and \f$ (R_2, S_2) \f$ encrypted
 * under public keys \f$ P_1 \f$ and \f$ P_2 \f$ respectively, the prover
 * demonstrates knowledge of scalars \f$ m, r_1, r_2 \f$ such that:
 ...
 * To prove that \f$ m \f$ is identical in both ciphertexts without revealing
 * it, the prover uses a **shared random nonce** \f$ k_m \f$ for the amount
 * commitment across both logical branches of the proof.
 *
 * 1. **Commitments:**
 ...
 * ciphertexts use the **same** \f$ s_m \f$ and \f$ T_m \f$, mathematically
 * enforcing equality:
 * - \f$ s_m \cdot G + s_{r1} \cdot P_1 \stackrel{?}{=} T_m + T_{r1,P1} + e
 * \cdot S_1 \f$
 * - \f$ s_m \cdot G + s_{r2} \cdot P_2 \stackrel{?}{=} T_m + T_{r2,P2} + e
 * \cdot S_2 \f$
 *
 * **Security Context:**
 * This is used when transferring confidential tokens between accounts
 * (re-encrypting the sender's balance for the receiver) or updating keys,
 * ensuring no value is created or destroyed during the transformation.
 *
...
int secp256k1_mpt_prove_same_plaintext(
    ...
    const secp256k1_pubkey *R1, const secp256k1_pubkey *S1,
    const secp256k1_pubkey *P1, const secp256k1_pubkey *R2,
    const secp256k1_pubkey *S2, const secp256k1_pubkey *P2, uint64_t amount_m,
    const unsigned char *randomness_r1, const unsigned char *randomness_r2,
    const unsigned char *tx_context_id)
{
  ...
  if (!generate_random_scalar(ctx, k_m))
    goto cleanup;
  if (!generate_random_scalar(ctx, k_r1))
    goto cleanup;
  if (!generate_random_scalar(ctx, k_r2))
    goto cleanup;
  ...
  if (!secp256k1_ec_pubkey_create(ctx, &T_r1_G, k_r1))
    goto cleanup;
  T_r1_P1 = *P1;
  if (!secp256k1_ec_pubkey_tweak_mul(ctx, &T_r1_P1, k_r1))
    goto cleanup;

  if (!secp256k1_ec_pubkey_create(ctx, &T_r2_G, k_r2))
    goto cleanup;
  T_r2_P2 = *P2;
  if (!secp256k1_ec_pubkey_tweak_mul(ctx, &T_r2_P2, k_r2))
    goto cleanup;

  /* 3. Challenge */
  build_same_plaintext_hash_input(ctx, e, R1, S1, P1, R2, S2, P2, &T_m, &T_r1_G,
  ...
  if (!secp256k1_ec_seckey_tweak_mul(ctx, term, e))
    goto cleanup;
  if (!secp256k1_ec_seckey_tweak_add(ctx, s_r1, term))
    goto cleanup;
  ...
  if (!secp256k1_ec_seckey_tweak_mul(ctx, term, e))
    goto cleanup;
  if (!secp256k1_ec_seckey_tweak_add(ctx, s_r2, term))
    goto cleanup;
  ...
  len = 33;
  if (!secp256k1_ec_pubkey_serialize(ctx, ptr, &len, &T_m,
                                     SECP256K1_EC_COMPRESSED))
    goto cleanup;
  ptr += 33;
  len = 33;
  if (!secp256k1_ec_pubkey_serialize(ctx, ptr, &len, &T_r1_G,
                                     SECP256K1_EC_COMPRESSED))
    goto cleanup;
  ptr += 33;
  len = 33;
  if (!secp256k1_ec_pubkey_serialize(ctx, ptr, &len, &T_r1_P1,
                                     SECP256K1_EC_COMPRESSED))
    goto cleanup;
  ptr += 33;
  len = 33;
  if (!secp256k1_ec_pubkey_serialize(ctx, ptr, &len, &T_r2_G,
                                     SECP256K1_EC_COMPRESSED))
    goto cleanup;
  ptr += 33;
  len = 33;
  if (!secp256k1_ec_pubkey_serialize(ctx, ptr, &len, &T_r2_P2,
                                     SECP256K1_EC_COMPRESSED))
    goto cleanup;
  ptr += 33;
    ...
    const unsigned char *tx_context_id)
{
  ...
  if (!secp256k1_ec_pubkey_parse(ctx, &T_m, ptr, 33))
    goto cleanup;
  ptr += 33;
  if (!secp256k1_ec_pubkey_parse(ctx, &T_r1_G, ptr, 33))
    goto cleanup;
  ptr += 33;
  if (!secp256k1_ec_pubkey_parse(ctx, &T_r1_P1, ptr, 33))
    goto cleanup;
  ptr += 33;
  if (!secp256k1_ec_pubkey_parse(ctx, &T_r2_G, ptr, 33))
    goto cleanup;
  ptr += 33;
  if (!secp256k1_ec_pubkey_parse(ctx, &T_r2_P2, ptr, 33))
    goto cleanup;
  ptr += 33;
  ...
  if (!secp256k1_ec_seckey_verify(ctx, s_m))
    goto cleanup;
  if (!secp256k1_ec_seckey_verify(ctx, s_r1))
    goto cleanup;
  if (!secp256k1_ec_seckey_verify(ctx, s_r2))
    goto cleanup;
  ...
  if (!secp256k1_ec_pubkey_create(ctx, &LHS, s_r1))
    goto cleanup;
  term = *R1;
  ...
  pts[0] = &T_r1_G;
  pts[1] = &term;
  if (!secp256k1_ec_pubkey_combine(ctx, &RHS, pts, 2))
    goto cleanup;
  ...
  term = *P1;
  if (!secp256k1_ec_pubkey_tweak_mul(ctx, &term, s_r1))
    goto cleanup;
  pts[0] = &SmG;
  pts[1] = &term;
  ...
  pts[1] = &T_r1_P1;
  pts[2] = &term;
  if (!secp256k1_ec_pubkey_combine(ctx, &RHS, pts, 3))
    goto cleanup;
  if (!pubkey_equal(ctx, &LHS, &RHS))
    goto cleanup;

  /* Eq 3: s_r2 * G == T_r2_G + e * R2 */
  if (!secp256k1_ec_pubkey_create(ctx, &LHS, s_r2))
    goto cleanup;
  term = *R2;
  if (!secp256k1_ec_pubkey_tweak_mul(ctx, &term, e))
    goto cleanup;
  ...
  term = *P2;
  if (!secp256k1_ec_pubkey_tweak_mul(ctx, &term, s_r2))
    goto cleanup;
  pts[0] = &SmG;
  pts[1] = &term;
  if (!secp256k1_ec_pubkey_combine(ctx, &LHS, pts, 2))
