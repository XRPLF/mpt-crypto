 ...
 *
 * Given a public key \f$ P = sk \cdot G \f$, the protocol proves knowledge of
 * \f$ sk \f$. It is made non-interactive using the Fiat-Shamir transform.
 *
 * 1. **Commitment:**
 ...
 * \f[ e = H(\text{"MPT_POK_SK_REGISTER"} \parallel P \parallel T \parallel
 * \text{ContextID}) \f]
 *
 * 3. **Response:**
                               ...
                               const unsigned char *context_id)
{
  ...
  if (!secp256k1_ec_seckey_verify(ctx, sk))
    return 0;
  if (!generate_random_scalar(ctx, k))
    goto cleanup;
    ...
    const secp256k1_pubkey *pk, const unsigned char *context_id)
{
  ...
  /* 1. Parse T (33 bytes) */
  if (!secp256k1_ec_pubkey_parse(ctx, &T, ptr, 33))
    goto cleanup;
  ptr += 33;
