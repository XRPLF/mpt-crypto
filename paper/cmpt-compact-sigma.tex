...
\begin{remark}[Deterministic verification]
  ...
  reveals~$r$ directly via the \texttt{BlindingFactor} field and the
  verifier checks the ciphertext deterministically, at the cost of
  $32$~bytes with no sigma protocol.
  In the production CMPT specification, this is the adopted path: the
  prover discloses~$r$ and the verifier checks $C_1 = r \cdot G$ and
  $C_2 = m \cdot G + r \cdot P_A$.
  If this path is adopted, compact form is moot for these two
  transaction types.
  The compact sigma analysis below is retained for completeness.
\end{remark}
...
\begin{description}[style=nextline,leftmargin=1.5em]
  \item[Convert / Clawback (compact $\Pi_{\mathrm{plain}}$).]
    ...
    Send compact tag (\texttt{CMPT\_SEND\_SIGMA}) and the
...
\begin{enumerate}
  \item \textbf{Proof-size constants.}
    ...
    $64$~bytes for Convert/Clawback sigma proofs and $128$~bytes for
