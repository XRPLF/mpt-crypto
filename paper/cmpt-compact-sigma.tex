...
\begin{multline}
  \label{eq:fs-hash}
  e = \mathcal{H}\!\bigl(
    \texttt{"CMPT\_COMBINED\_STD\_PROOF"} \;\|\;
    P_1 \| \cdots \| P_n \;\|\;
    C_1 \| C_{2,1} \| \cdots \| C_{2,n} \;\|\; \\
    \mathit{PC}_m \| P_A \| \mathit{PC}_b \| B_1 \| B_2 \;\|\;
...
\begin{definition}[Compact-form proof $\Pi_{\mathrm{send}}$]
  ...
  \[
    \Pi_{\mathrm{send}} = (e,\, z_r,\, z_m,\, z_{\sk},\, z_\rho,\, z_b)
    \;\in\; \Zq^6.
  \]
  \textbf{Verification:}
  \begin{enumerate}
...
\begin{enumerate}
  ...
  \item All first-round commitments
        $T_1, T_{2,1}, \ldots, T_{2,n}, T_m, T_{\sk,1}, T_b, T_{\sk,2}$.
...
\subsection{Convert and Clawback}
\label{sec:convert-clawback}

In \textsf{ConfidentialMPTConvert}, a holder moves a publicly known
amount~$m$ from cleartext into a confidential balance.
The holder proves knowledge of the randomness~$r$ used in the
ciphertext $(C_1, C_2) = (r \cdot G,\; m \cdot G + r \cdot P_A)$.
Since $m$ is public, this is a single Schnorr proof of knowledge
of~$r$ ($\Pi_{\mathrm{plain}}$), currently serialised in commitment
form as $(T_1, T_2, s) \in \G^2 \times \Zq$ costing
$2 \times 33 + 32 = 98$~bytes.
...
\textsf{ConfidentialMPTClawback} is structurally identical: the issuer
reveals $m$ and proves knowledge of randomness in a mirror ciphertext.
Compact form yields the same $64$-byte proof.

\begin{remark}[Deterministic verification]
  \label{rem:deterministic}
  Both Convert and Clawback admit an alternative where the prover
  reveals~$r$ directly and the verifier checks the ciphertext
  deterministically, at the cost of $32$~bytes with no sigma protocol.
  If this path is adopted, compact form is moot for these two
  transaction types.
  The analysis below assumes ZK proofs are used.
\end{remark}
...
The proof bundle has two sigma components:
\begin{enumerate}[label=(\roman*)]
  \item $\Pi_{\mathrm{plain}}$: proof that the withdrawal ciphertext
        encrypts the declared~$m$ ($98$~bytes in commitment form).
  \item $\Pi_{\mathrm{link\text{-}B}}$: balance linkage
        (Variant~B)---the existing balance ciphertext $(B_1, B_2)$
        encodes the same value~$b$ committed in $\mathit{PC}_b$
        ($195$~bytes in commitment form).
\end{enumerate}
A single-value Bulletproof ($688$~bytes) then proves
$b - m \in [0, 2^{64})$.

\subsubsection*{Combined relation}

Let $r_w$ denote the fresh randomness for the withdrawal ciphertext,
$(C_{1,w}, C_{2,w}) = (r_w \cdot G,\; m \cdot G + r_w \cdot P_A)$.
The combined ConvertBack relation is
\begin{equation}
  \label{eq:convertback-relation}
  \mathcal{R}_{\mathrm{cb}} = \left\{
  \bigl(\mathbf{x},\; w\bigr)
  \;\middle|\;
  \begin{aligned}
    C_{1,w} &= r_w \cdot G, \\
    C_{2,w} &= m \cdot G + r_w \cdot P_A, \\
    P_A &= \sk_A \cdot G, \\
    B_2 - b \cdot G &= \sk_A \cdot B_1, \\
    \mathit{PC}_b &= b \cdot G + \rho \cdot H
  \end{aligned}
  \right\},
\end{equation}
where $w = (r_w,\, b,\, \sk_A,\, \rho)$ and $m$ is a public input.

\subsubsection*{AND-composed compact-form protocol}

The prover samples nonces $(t_{r_w},\, t_b,\, t_{\sk},\, t_\rho)
\xleftarrow{\$} \Zq^4$ and computes five commitments:
\begin{alignat*}{2}
  T_1 &= t_{r_w} \cdot G, &\qquad
  T_2 &= t_{r_w} \cdot P_A, \\
  T_{\sk,1} &= t_{\sk} \cdot G, &\qquad
...
five commitments, and responses are
$z_{r_w} = t_{r_w} + e \cdot r_w$,
$z_b = t_b + e \cdot b$,
...
\begin{definition}[Compact-form proof $\Pi_{\mathrm{cb}}$]
  ...
  The non-interactive proof in compact form is
  \[
    \Pi_{\mathrm{cb}} = (e,\, z_{r_w},\, z_b,\, z_{\sk},\, z_\rho)
    \;\in\; \Zq^5.
  \]
  \textbf{Verification:}
  \begin{enumerate}
    ...
    \begin{align}
      T_1 &= z_{r_w} \cdot G - e \cdot C_{1,w},
        \label{eq:recon-cb-t1} \\
      T_2 &= z_{r_w} \cdot P_A - e \cdot (C_{2,w} - m \cdot G),
        \label{eq:recon-cb-t2} \\
      T_{\sk,1} &= z_{\sk} \cdot G - e \cdot P_A,
    ...
    \end{align}
    \item Recompute $e'$ from the Fiat--Shamir hash over the domain
          tag, public inputs, and reconstructed commitments.
    \item Accept iff $e' = e$.
  \end{enumerate}
\end{definition}

\noindent\textbf{Proof size.}
$5 \times 32 = \mathbf{160}$~bytes (one challenge, four responses).
The current commitment-form cost is $98 + 195 = 293$~bytes.
The saving decomposes into two independent contributions:
\begin{itemize}
  \item \emph{Compact form} (dropping group-element commitments):
        $293 - 192 = 101$~bytes.
  \item \emph{AND composition} (merging two separate challenges into
        one): a further $192 - 160 = 32$~bytes.
\end{itemize}

\begin{remark}[Security]
  The security argument for $\Pi_{\mathrm{cb}}$ is analogous to
  \cref{sec:security}: the witnesses $(r_w, b, \sk_A, \rho)$ are
...
\begin{table}[htbp]
  ...
  \caption{Sigma-proof sizes across transaction types.
    ...
    Convert and Clawback assume ZK proofs rather than deterministic
    verification (\cref{rem:deterministic}).}
  \label{tab:cross-tx}
...
For \textsf{ConvertBack}, the $133$-byte reduction still represents a
meaningful improvement: the total proof payload drops from
$293 + 688 = 981$~bytes to $160 + 688 = 848$~bytes ($14\%$).
...
The different transaction types compose these building blocks: Convert
and Clawback use $\Pi_{\mathrm{plain}}$ alone; ConvertBack uses
$\Pi_{\mathrm{plain}} + \Pi_{\mathrm{link\text{-}B}}$ plus a
...
\begin{description}[style=nextline,leftmargin=1.5em]
  \item[Convert / Clawback (compact $\Pi_{\mathrm{plain}}$).]
    A new source file (e.g.\
    \texttt{proof\_compact\_convert.c}) providing prove/verify
    entry points that replace the commitment-form
    \texttt{secp256k1\_equality\_plaintext} calls on main.
    The proof shrinks from $98$ to $64$~bytes.
    Internally, the prover samples one nonce~$t_{r}$, computes one
    response $z_r = t_r + e \cdot r$, and serialises $(e, z_r)$.
    The verifier reconstructs $T_1, T_2$ via
    \crefrange{eq:recon-conv-t1}{eq:recon-conv-t2} and re-hashes.
    %
    The Fiat--Shamir domain tag must differ from both the
    Send compact tag (\texttt{CMPT\_COMBINED\_STD\_PROOF}) and the
    existing commitment-form tag
    (\texttt{MPT\_POK\_PLAINTEXT\_PROOF}) to prevent cross-protocol
    forgery.
  ...
  \item[ConvertBack (compact $\Pi_{\mathrm{cb}}$).]
    ...
    This AND-composes the $\Pi_{\mathrm{plain}}$ and
    $\Pi_{\mathrm{link\text{-}B}}$ relations under a single
    Fiat--Shamir challenge, replacing the two separate
    building-block calls on main
    (plaintext PoK and ElGamal--Pedersen linkage).
    The implementation mirrors the Send compact proof: sample four
    nonces, compute the five commitments of
    \cref{sec:convertback}, derive~$e$, compute four responses,
    and serialise $(e, z_{r_w}, z_b, z_{\sk}, z_\rho)$.
    A new domain tag (distinct from both the Send compact tag and
    the existing commitment-form tags) is required.
...
\begin{enumerate}
  \item \textbf{Proof-size constants.}
    ...
    $64$~bytes for Convert/Clawback sigma proofs and $160$~bytes for
    ConvertBack sigma proofs (replacing $98$ and $98 + 195 = 293$
    respectively).
  ...
  \item \textbf{Verification call sites.}
    ...
    For Convert and Clawback, a single call replaces the existing
    one, with the only visible change being the proof buffer size.
