...
\begin{remark}[Deterministic verification for Convert]
  \label{rem:deterministic}
  Convert admits an alternative where the prover
  ...
  If this path is adopted, compact form is moot for Convert.
  Clawback does not admit this alternative, since the issuer proves
  knowledge of~$\sk_{\mathrm{iss}}$, not randomness.
\end{remark}
...
\begin{table}[htbp]
  ...
  \caption{Sigma-proof sizes across transaction types.
    ...
    Convert assumes a ZK proof rather than deterministic
...
uses $\Pi_{\mathrm{plain}}$ alone; Clawback uses a Chaum--Pedersen
...
\begin{description}[style=nextline,leftmargin=1.5em]
  \item[Convert (compact $\Pi_{\mathrm{plain}}$).]
...
ConvertBack, Convert, and Clawback.
